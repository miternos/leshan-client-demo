...
public class LeshanClientDemo {
    ...
    public static void main(final String[] args) {
        ...
        options.addOption("device", true, "IPSO Device Type Id (3303 for humidity or 3304 for Temp)");
        options.addOption("src", true, "Source File Location");
        options.addOption("frq", true, "Frequency in seconds (default: 10)");
        options.addOption("val", true, "Start value");
        options.addOption("min", true, "Min value");
        options.addOption("max", true, "Max value");
        ...
        if (!cl.hasOption("src") && (!cl.hasOption("val") || !cl.hasOption("min") || !cl.hasOption("max"))) {
            System.err.println("When src is not provided, start, min, max are mandatory : " + cl.getArgList());
            formatter.printHelp(USAGE, options);
            return;
        }

        if (!cl.hasOption("device")) {
            System.err.println("Device type is mandatory: " + cl.getArgList());
            formatter.printHelp(USAGE, options);
            return;
        }
        ...
        String source = cl.getOptionValue("src");

        String device = cl.getOptionValue("device");
        ...
        if (StringUtils.isEmpty(source)){
            ...
            start = Double.valueOf(cl.getOptionValue("val"));
            min = Double.valueOf(cl.getOptionValue("min"));
            max = Double.valueOf(cl.getOptionValue("max"));
        }

        long period = 10;
        if (cl.hasOption("frq")){
            period = Long.valueOf(cl.getOptionValue("frq"));
        }

        createAndStartClient(endpoint, localAddress, localPort, secureLocalAddress, secureLocalPort, cl.hasOption("b"),
                             serverURI, pskIdentity, pskKey, latitude, longitude, scaleFactor, device, start, min, max, source, period);
    }

    public static void createAndStartClient(String endpoint,
                                            String localAddress,
                                            int localPort,
        ...
        if (Integer.valueOf(device) == OBJECT_ID_HUMIDITY_SENSOR) {
            initializer
                    .setInstancesForObject(OBJECT_ID_HUMIDITY_SENSOR, new RandomSensor("percentage", start, min, max, source, period));
            enablers = initializer.create(SECURITY, SERVER, DEVICE, LOCATION, OBJECT_ID_HUMIDITY_SENSOR);
        } else if (Integer.valueOf(device) == OBJECT_ID_TEMPERATURE_SENSOR) {
            initializer.setInstancesForObject(OBJECT_ID_TEMPERATURE_SENSOR, new RandomSensor("cel", start, min, max, source,period));
            enablers = initializer.create(SECURITY, SERVER, DEVICE, LOCATION, OBJECT_ID_TEMPERATURE_SENSOR);
        } else {
            System.err.println("Given device \"" + device + "\" is not supported !!!");
            return;
        }
